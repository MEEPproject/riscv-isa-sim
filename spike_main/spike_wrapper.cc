...
{    
    ...
    */
    SpikeWrapper::SpikeWrapper(std::string p, std::string t,  std::string ic, std::string dc, std::string isa, std::string cmd, std::string varch, bool fast_cache, bool enable_smart_mcpu, bool vector_bypass_l1, bool vector_bypass_l2) :
            p_(p),
            t_(t),
            ic_(ic),
            dc_(dc),
    ...
    identified or not. After executing, l1Misses (reference) will contain any CacheRequests that 
    are generated by the simulation of the instruction.
    */
            
    bool SpikeWrapper::simulateOne(uint16_t core, uint64_t current_cycle, std::list<std::shared_ptr<spike_model::Event>>& events)
    {
        return simulation->simulate_one(core, current_cycle, events);
    }
    ...
    */
    ...
    std::shared_ptr<CacheRequest> SpikeWrapper::serviceCacheRequest(std::shared_ptr<CacheRequest> req, uint64_t timestamp)
    {
        ...
        if(req->getType()==CacheRequest::AccessType::FETCH)
        {
           ics[cache_idx]->serviceCacheRequest(req);
        }
        else
        {
           wb=dcs[cache_idx]->serviceCacheRequest(req);
        }
        if(wb!=nullptr)
        {
    ...
    */
	...
	std::cout << "Calling sim with smart mcpu " << enable_smart_mcpu << std::endl;
        std::shared_ptr<sim_t> s=std::make_shared<sim_t> (isa, priv, varch, nprocs, halted, start_pc, mems, plugin_devices, htif_args,
                std::move(hartids), dm_config, enable_smart_mcpu, vector_bypass_l1, vector_bypass_l2); //BORJA
