...
{    
    ...
    */
    SpikeWrapper::SpikeWrapper(std::string p, std::string t,  std::string ic, std::string dc, std::string isa, std::string cmd, std::string varch, bool fast_cache, bool enable_smart_mcpu, size_t scratchpad_size) :
            p_(p),
            t_(t),
            ic_(ic),
            dc_(dc),
    ...
    */
	...
	std::cout << "Calling sim with smart mcpu " << enable_smart_mcpu << std::endl;
        std::shared_ptr<sim_t> s=std::make_shared<sim_t> (isa, priv, varch, nprocs, halted, start_pc, mems, plugin_devices, htif_args,
                std::move(hartids), dm_config, enable_smart_mcpu, scratchpad_size); //BORJA
