...
{    
    ...
    */
    ...
    identified or not. After executing, l1Misses (reference) will contain any Requests that 
    are generated by the simulation of the instruction.
    */
            
    bool SpikeWrapper::simulateOne(uint16_t core, uint64_t current_cycle, std::list<std::shared_ptr<spike_model::SpikeEvent>>& events)
    {
        return simulation->simulate_one(core, current_cycle, events);
    }

    /*
    Notifies the completion of an Request, so the corresponding register is set as available.
    */
